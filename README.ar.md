...
# XXD Panel 184｜مشهد يومي مرسوم داخل مربع
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-184`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-184 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-184-prompt.en.md) أو [بالصينية](references/xxd-panel-184-prompt.zh-CN.md).
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 184 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
